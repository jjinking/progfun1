...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val resultMap = points.groupBy{findClosest(_, means)}
    means.foldLeft(resultMap)((acc: GenMap[Point, GenSeq[Point]], p: Point) => {
                                if (!acc.contains(p)) acc + (p -> GenSeq())
                                else acc
                              })
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).map {
      case (p: Point, q: Point) => {
        if (p.squareDistance(q) <= eta) true
        else false
      }
    }.fold(true)(_ && _) == true
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val class2Points = classify(points, means)
    val newMeans = update(class2Points, means)
