...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet = {
    if (that.contains(elem)) right.union(left.union(that))
  ...
  def mostRetweeted: Tweet = {
    ...
    List(leftMost, elem, rightMost).sortBy(t => -t.retweets).head
  }
...
object GoogleVsApple {
  ...
  def allTweetsFiltered(words:List[String]):TweetSet = {
  ...
  lazy val googleTweets: TweetSet = allTweetsFiltered(google)
  lazy val appleTweets: TweetSet = allTweetsFiltered(apple)
